...
{
    private const string path = "/ThirdParty/TabToy/";
    ...
    {
        string paths = Application.dataPath + "/../../svnExcelPath.txt";//Assets/的上两层
        if (File.Exists(paths))
        {
            paths = File.ReadAllText(paths);
        }
    ...
    {
        var p = Application.dataPath + path + "/GenerateClass/Config.cs";
